import { getConfig } from '@workos-inc/authkit-ssr';
import type {
 ...
 * @returns A function that can only be called once.
 */
function once<TArgs extends unknown[], TReturn>(fn: (...args: TArgs) => TReturn): (...args: TArgs) => TReturn {
  let called = false;
  let result: TReturn;
...
export class UserManagement {
  ...
  constructor(
    ...
    this.client = new HttpClient({
      ...
      defaultHeaders: {
        ...
        'User-Agent': options.appInfo ? `${options.appInfo.name}/${options.appInfo.version}` : 'authkit-ssr',
      },
    });
  }
  ...
  getLogoutUrl({ sessionId, returnTo }: { sessionId: string; returnTo?: string }): string {
    if (!sessionId) {
  ...
  async authenticateWithRefreshToken(options: AuthenticateWithRefreshTokenOptions): Promise<AuthenticationResponse> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data } = await this.client.post<any>('/user_management/authenticate', {
  ...
  async authenticateWithCode(options: AuthenticateWithCodeOptions): Promise<AuthenticationResponse> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data } = await this.client.post<any>('/user_management/authenticate', {
...
/**
 * WorkOSLite is a lightweight version of the WorkOS SDK without any node-specific dependencies.
 */
export class WorkOSLite {
